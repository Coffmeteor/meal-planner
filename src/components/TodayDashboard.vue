...
import { formatCalories, formatTime, formatDateYmd, parseYmd } from '../utils/helpers.js'
...
const calorieStatusColor = computed(() => {
  ...
  if (abs <= CALORIE_DEVIATION_MINOR) return '#34c759'
  if (abs <= CALORIE_DEVIATION_MAJOR) return '#ff9500'
  return '#ff3b30'
})
const calorieStatusLabel = computed(() => {
  ...
  if (abs <= CALORIE_DEVIATION_MAJOR) return '略有偏差'
  return '偏离较多'
})
  ...
  <section class="today-dashboard">
    ...
    <article class="today-card calorie-card">
      ...
      <div class="calorie-info">
        <div class="calorie-top">
          <span class="calorie-status" :style="{ color: calorieStatusColor }">{{
            calorieStatusLabel
          }}</span>
          <span
            v-if="calorieDeviation != null"
            class="calorie-dev"
            :style="{
              color:
                Math.abs(calorieDeviation) > CALORIE_DEVIATION_MODERATE
                  ? '#ff3b30'
                  : 'var(--color-text)',
            }"
          >
        ...
        <span class="calorie-target"
          >目标 {{ targetCalories ? Math.round(targetCalories) : '--' }} kcal</span
        >
        <!-- Macro bars -->
        <div class="macro-bar-row">
          <div class="macro-bar-item">
            ...
            <div class="macro-bar-track">
              <div class="macro-bar-fill protein" :style="{ width: macroBars.protein + '%' }" />
            </div>
          ...
          <div class="macro-bar-item">
            ...
            <div class="macro-bar-track">
              <div class="macro-bar-fill carbs" :style="{ width: macroBars.carbs + '%' }" />
            </div>
          ...
          <div class="macro-bar-item">
            ...
            <div class="macro-bar-track">
              <div class="macro-bar-fill fat" :style="{ width: macroBars.fat + '%' }" />
            </div>
    ...
    <article class="today-card">
        ...
        >
          ...
          <div class="meal-actions">
            <button
              ...
              :class="{ active: meal.locked }"
              @click.stop="
                meal.locked
                  ? emit('unlock-meal', { dayIndex: todayIndex, mealIndex })
                  : emit('lock-meal', { dayIndex: todayIndex, mealIndex })
              "
              :title="meal.locked ? '解锁' : '锁定'"
            >
              {{ meal.locked ? '🔒' : '🔓' }}
            </button>
            <button
              class="meal-action-btn edit"
              @click.stop="openMealEditor(mealIndex)"
              title="编辑"
            >
              ✎
            </button>
      ...
      <div class="card-footer">
        ...
        <button class="footer-btn accent" @click="emit('optimize-day', todayIndex)">
          优化热量
        </button>
      </div>
    </article>
          ...
          <div class="status-text">
            ...
            <span>{{
              hasTodayWeight
                ? '已完成'
                : latestWeight != null
                  ? `最近 ${Math.round(latestWeight * 10) / 10} kg`
                  : '待记录'
            }}</span>
          </div>
...
.macro-mini-dot.protein {
  background: #34c759;
}
.macro-mini-dot.carbs {
  background: #ff9500;
}
.macro-mini-dot.fat {
  background: #ffd60a;
