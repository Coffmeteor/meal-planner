...
const recentAverageWeight = computed(() => {
  ...
  return (
    Math.round((recent.reduce((sum, log) => sum + Number(log.weight), 0) / recent.length) * 10) / 10
  )
})
...
const latestBodyMeasurements = computed(() => {
  ...
  for (const f of fields) {
    if (last[f] != null && Number(last[f]) > 0) {
      values[f] = Number(last[f])
      hasAny = true
    }
  }
...
<template>
  <section class="tab-page-shell progress-page">
    ...
    <section class="progress-dashboard-card weight-hero-card">
      <div class="progress-card-head">
        <div>
          <p>体重</p>
          <h2 class="big-number">
            {{ currentWeight == null ? '--' : formatKg(currentWeight) }}<small> kg</small>
          </h2>
        </div>
      ...
      <button type="button" class="ghost-action full-width" @click="emit('recordWeight')">
    ...
    <section v-if="latestBodyMeasurements" class="progress-dashboard-card body-measure-card">
      ...
      <div class="stat-row stat-row-2col">
        <div v-if="latestBodyMeasurements.waist" class="stat-cell">
          <span>腰围</span><strong>{{ latestBodyMeasurements.waist }}<small>cm</small></strong>
        </div>
        <div v-if="latestBodyMeasurements.hip" class="stat-cell">
          <span>臀围</span><strong>{{ latestBodyMeasurements.hip }}<small>cm</small></strong>
        </div>
        <div v-if="latestBodyMeasurements.chest" class="stat-cell">
          <span>胸围</span><strong>{{ latestBodyMeasurements.chest }}<small>cm</small></strong>
        </div>
        <div v-if="latestBodyMeasurements.bodyFat" class="stat-cell">
          <span>体脂率</span><strong>{{ latestBodyMeasurements.bodyFat }}<small>%</small></strong>
        </div>
        <div v-if="latestBodyMeasurements.thigh" class="stat-cell">
          <span>大腿围</span><strong>{{ latestBodyMeasurements.thigh }}<small>cm</small></strong>
        </div>
        <div v-if="latestBodyMeasurements.arm" class="stat-cell">
          <span>上臂围</span><strong>{{ latestBodyMeasurements.arm }}<small>cm</small></strong>
        </div>
      </div>
      <button type="button" class="ghost-action full-width" @click="emit('recordWeight')">
        记录身体数据
