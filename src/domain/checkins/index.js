...
export function analyzeRecentCheckins(checkins) {
  ...
  return {
    ...
    avgAdherence: adherenceLogs.length
      ? Math.round(
          (adherenceLogs.reduce((sum, log) => sum + log.adherenceLevel, 0) / adherenceLogs.length) *
            10,
        ) / 10
      : null,
...
export function generateCheckinAdvice(checkins, weightTrend) {
  ...
  if (analysis.ateOutCount >= 3) {
    advice.push(
      `外食较多（${analysis.ateOutCount}天/周），建议给外食日预留更简单的餐单，不要用全自炊标准硬套。`,
    )
  }

  if (analysis.exerciseCount <= 1) {
    advice.push(
      `运动记录偏少（${analysis.exerciseCount}次/周），可以先增加日常步行或低门槛活动，不必一开始追求高强度训练。`,
    )
  }

  if (analysis.avgSleep != null && analysis.avgSleep <= 2.5) {
    advice.push(
      `睡眠偏弱（平均${analysis.avgSleep}/5），体重波动可能更大，建议先稳定作息再判断是否需要调热量。`,
    )
  }
