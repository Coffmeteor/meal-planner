...
export function analyzeWeightTrend(logs) {
  ...
  const avg7 =
    Math.round((recent7.reduce((sum, log) => sum + Number(log.weight), 0) / recent7.length) * 10) /
    10
...
export function generateAdvice(logs, profile = null) {
  ...
  const avgHunger =
    hungerLogs.reduce((sum, log) => sum + Number(log.hungerLevel), 0) /
    Math.max(hungerLogs.length, 1)
  const avgAdherence =
    adherenceLogs.reduce((sum, log) => sum + Number(log.adherenceLevel), 0) /
  ...
  if (trend.trend === 'stable' && valid.length >= 5) {
    ...
    if (times.length >= 2) {
      ...
      if (daysCovered > 10 * 86400000) {
        advice.push(
          '近 10 天以上体重无明显变化，可能进入平台。建议先检查执行度；如果执行稳定，可减少约 100 kcal 或增加日常活动。',
        )
      }
    }
  }
  ...
  if (avgHunger >= 4) {
    advice.push(
      `饥饿感偏高（平均${Math.round(avgHunger * 10) / 10}/5），建议优先增加蔬菜体积、蛋白质或调整餐次，不建议继续加大缺口。`,
    )
  }

  if (avgAdherence <= 3 && avgAdherence > 0) {
    advice.push(
      `执行度偏低（平均${Math.round(avgAdherence * 10) / 10}/5），建议降低计划难度，选择更容易执行的餐单。`,
    )
  }
...
export function buildWeightChartData(weightLogs, profile, planDays, startDate) {
  ...
  const chartStart =
    targetPoints[0]?.date && targetPoints[0].date < actualPoints[0].date
      ? targetPoints[0].date
      : actualPoints[0].date
