...
import {
  ...
  mealProfiles,
  splitMap,
  CALORIE_CALIBRATION_THRESHOLD,
  CALORIE_DEVIATION_MINOR,
  PORTION_SCALE_MIN,
  PORTION_SCALE_MAX,
  PORTION_RECALIBRATION_MIN,
  PORTION_STEP,
} from './constants.js'
...
export function generateScheduleFromProfile(profile, dietMethod, existingEatingWindow = null) {
  ...
  return {
    ...
    times: [deriveTimeAfterWake(wakeTime, 0.5), '12:30', deriveTimeBeforeSleep(sleepTime, 3.5)],
    split: [0.3, 0.4, 0.3],
    eatingWindow,
  }
}
...
function buildMainMeal(category, calorieTarget, offset, foodPool, options = {}) {
  ...
  const staple =
    staples[(offset + 1) % staples.length] || activePool[(offset + 1) % activePool.length]
  const vegetable =
  ...
  const scale = Math.min(
    PORTION_SCALE_MAX,
    Math.max(PORTION_SCALE_MIN, calorieTarget / Math.max(baseCalories, 1)),
  )

  return base.map((food) =>
    scaleFood(food, Math.round((food.defaultPortion * scale) / PORTION_STEP) * PORTION_STEP),
  )
}
...
function buildSnackMeal(calorieTarget, offset, foodPool, options = {}) {
  ...
  const scale = Math.min(
    PORTION_SCALE_MAX,
    Math.max(0.6, calorieTarget / Math.max(baseCalories, 1)),
  )
...
function resolveEatingWindow(params = {}, schedule = {}, eatingWindow = null) {
  ...
  const method = rawWindow?.type && rawWindow.type !== 'none' ? rawWindow.type : params?.dietMethod
...
export function generateMealPlan(
  ...
  return Array.from({ length: days }, (_, dayIndex) => {
    ...
    const calibratedMeals =
      Math.abs(deviation) > CALORIE_CALIBRATION_THRESHOLD
        ? calibrateDayMeals(
            meals,
            deviation,
...
export function regenerateSingleMeal(
  ...
  const calorieTarget =
    (Number(params?.targetCalories) || 1500) * (splits[mealIndex] || 1 / Math.max(mealCount, 1))
  const foodPool = normalizeFoodPool(availableFoods?.length ? availableFoods : foods)
...
export function regenerateDay(
  ...
  if (Math.abs(deviation) > 80) {
    const calibrated = calibrateDayMeals(meals, deviation, dailyCalories)
    if (calibrated !== meals) {
...
function calibrateDayMeals(meals, deviation, dailyTargetCalories) {
  ...
  const needed = Math.max(CALORIE_DEVIATION_MINOR, dailyTargetCalories - lockedCals)
  const ratio = Math.min(PORTION_SCALE_MAX, Math.max(PORTION_RECALIBRATION_MIN, needed / adjCals))

  const newMeals = meals.map((meal) => {
    if (meal.locked || meal.edited) return meal
    if (!meal.foods?.length) return meal
    const scaledFoods = meal.foods.map((f) => {
      const newPortion = Math.round((f.portion * ratio) / PORTION_STEP) * PORTION_STEP
      const pct = newPortion / Math.max(f.portion, 1)
      return {
        ...f,
        portion: newPortion,
        calories: Math.round(f.calories * pct),
        protein: Math.round(f.protein * pct),
        carbs: Math.round(f.carbs * pct),
        fat: Math.round(f.fat * pct),
      }
    })
