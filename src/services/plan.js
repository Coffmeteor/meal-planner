...
export function createPlan(params, schedule, availableFoods, eatingWindow) {
  ...
  return generateMealPlan(params, schedule, availableFoods, eatingWindow, nextPlanSeed())
}
...
export function optimizeTodayCalories(plan, dayIndex, targetCalories) {
  ...
  const adjustableCalories = adjustableMeals.reduce(
...
function scaleMeal(meal, ratio) {
  const scaleFood = (food) => {
    const portion = Math.max(
      1,
      Math.round((Number(food.portion || food.defaultPortion || 100) * ratio) / 5) * 5,
    )
  ...
  return {
    ...
    fat: totals.fat,
    portion: foods
      .map((food) => `${food.name}${Math.round(food.portion || 0)}${food.unit || 'g'}`)
      .join(' + '),
    edited: true,
  }
}
